<template>
  <section class="section">
    <div class="container">
      <div class="columns">
        <div class="column is-4 is-offset-4">

          <form class="paymentForm mt-5" action="./" method="POST"  @submit.prevent="payment">
            <div class="paymentForm__content">
                <div class="text-center">
                    <img  src="data:image/png;base64," width="180" height="50" alt="">
                </div>
                <div class="paymentForm__header mt-2">TẠO THANH TOÁN</div>
                <label for="">Loại thanh toán</label>
                    <select class="paymentForm__input" v-model="orderCategory" required>
                        <option v-for="(option, index) in orderCategoryOptions" :key="index" :value="option.value">
                            {{ option.text }}
                        </option>
                    </select>
                    <label for="">Số tiền cần thanh toán(VND):</label>
                    <input class="paymentForm__input" type="text" :value="transfer"  placeholder="Amount" required>
                    <label for="">Nội dung thanh toán:</label>
                    <textarea class="paymentForm__input" type="text" v-model="orderDesc"  placeholder="Description" required></textarea>
                    <label for="">Ngân hàng:</label>
                    <select class="paymentForm__input" v-model="vnp_BankCode" required>
                        <option v-for="(option, index) in bankCodeOptions" :key="index" :value="option.value">
                            {{ option.text }}
                        </option>
                    </select>
                    <label for="">Ngôn ngữ</label>
                    <select class="paymentForm__input" v-model="locale" required>
                        <option value="">Please select one</option>
                        <option value="vn">Tiếng Việt</option>
                        <option value="en">English</option>
                    </select>
                <button class="paymentForm__button" type="submit">Thanh toán</button>
            </div>
          </form>
        </div>
      </div>
    </div>
  </section>
</template>

<style>
    .paymentForm,
    .paymentForm * {
        box-sizing: border-box;
        font-family: 'Source Sans Pro';
    }

    .paymentForm {
        max-width: 350px;
        margin: 0 auto;
        border-radius: 5px;
        overflow: hidden;
        box-shadow: 0 0 15px rgba(0, 0, 0, 0.15);
    }

    .paymentForm__logo-container {
        padding: 30px;
        background: rgba(0, 0, 0, 0.25);
    }

    .paymentForm__logo {
        display: block;
        max-width: 125px;
        margin: 0 auto;
    }

    .paymentForm__content {
        padding: 30px;
        background: white;
        
    }

    .paymentForm__header {
        margin-bottom: 15px;
        text-align: center;
        color: #333333;
        font-weight: bold;
        font-size: 25px;
    }

    .paymentForm__input {
        width: 100%;
        margin-bottom: 10px;
        padding: 10px;
        border-radius: 5px;
        border: 2px solid #dddddd;
        background: white;
        outline: none;
        transition: border-color 0.5s;
    }

    .paymentForm__input:focus {
        border-color: white;
    }

    .paymentForm__input::placeholder {
        color: #aaaaaa;
    }

    .paymentForm__button {
        padding: 10px;
        color: #ffffff;
        font-weight: bold;
        background: #009578;
        width: 100%;
        border: none;
        outline: none;
        border-radius: 5px;
        cursor: pointer;
    }

    .paymentForm__button:active {
        background: #008067;
    }

    .paymentForm__links {
        margin-top: 15px;
        text-align: center;
    }

    .paymentForm__link {
        font-size: 0.9em;
        color: #008067;
        text-decoration: none;
    }

</style>
<script>
export default {
    async asyncData({ $axios,params}) {
        const bill = await $axios.$get(`api/CartDetails/SearchBill?BillId=${params.Payment}`)
        return { bill }
    },
    layout: 'ordersuccesslayout',
    middleware: ['isAuthorize'],
    data(){
        return{
            listcart: [],
            listBill: [],
            orderDesc: '',
            locale: '',
            orderCategory: '',
            orderCategoryOptions: [
                { text: 'Please select one', value: '' },
                { text: 'Thanh toán hóa đơn', value: 'billpayment' },
                { text: 'Thanh toán trực tuyến', value: 'other' }
            ],
            vnp_BankCode: '',
            bankCodeOptions: [
                { text: 'Please select one', value: '' },
                { value: "VNPAYQR", text: "VNPAYQR"},
                { value: "VNBANK", text: "LOCAL BANK"},
                { value: "INTCARD", text: "INTERNATIONAL CARD"},
                { value: "VISA", text: "VISA"},
                { value: "MASTERCARD", text: "MASTERCARD"},
                { value: "JCB", text: "JCB"},
                { value: "UPI", text: "UPI"},
                { value: "NCB", text: "Ngan hang NCB"},
                { value: "SACOMBANK", text: "Ngan hang SacomBank"},
                { value: "EXIMBANK", text: "Ngan hang EximBank"},
                { value: "MSBANK", text: "Ngan hang MSBANK"},
                { value: "NAMABANK", text: "Ngan hang NamABank "},
                { value: "VNMART", text: "Vi dien tu VNPAY"},
                { value: "VIETINBANK", text: "Ngan hang Vietinbank"},
                { value: "VIETCOMBANK", text: "Ngan hang VCB"},
                { value: "HDBANK", text: "Ngan hang HDBank"},
                { value: "DONGABANK", text: "Ngan hang Dong A"},
                { value: "TPBANK", text: "Ngân hàng TPBank"},
                { value: "OJB", text: "Ngân hàng OceanBank"},
                { value: "BIDV", text: "Ngân hàng BIDV"},
                { value: "TECHCOMBANK", text: "Ngân hàng Techcombank"},
                { value: "VPBANK", text: "Ngan hang VPBank"},
                { value: "AGRIBANK", text: "Ngan hang Agribank"},
                { value: "ACB", text: "Ngan hang ACB"},
                { value: "OCB", text: "Ngan hang Phuong Dong"},
                { value: "SCB", text: "Ngan hang SCB"},
            ],
            referenceCode: ''
        }
    },
    async created(){
    },
    computed: {
        total() {
            let total = 0;
            for(let i = 0; i < this.bill.length; i++) {
                total += this.bill[i].price * this.bill[i].quantity;
            }
                return total;
        },
        transfer(){
            let transfer = 0;
            for(let i = 0; i < this.bill.length; i++) {
                transfer += this.bill[i].price * this.bill[i].quantity * 23000;
            }
                return transfer;
        },

    },
    methods:{
        async payment() {
            try {
                const res = await this.$axios.post('/api/VNPay', {
                            orderCategory: this.orderCategory,
                            amount: this.transfer,
                            orderDesc: this.orderDesc,
                            vnp_BankCode: this.vnp_BankCode,
                            locale: this.locale,
                            referenceCode : this.bill[0].billId
                })
                window.location.href = res.data
            } catch (e) {
                console.log(e)
            }
        },

    }
}
</script>